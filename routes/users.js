...
//http://localhost:4000/users/searchuser/1
...
router.get("/searchuser/:userId", async (req, res) => {
  try {
    ...
    const response = await db
      .promise()
      .query(
        `SELECT name, email, username FROM users WHERE userid = '${userId}'`
      );
...
router.post("/adduser", async (req, res) => {
  try {
    ...
    const response = await db.promise()
      .query(`INSERT INTO users (userid,username,name,email,password,mobile )
...
router.put("/updateuserpassword", async (req, res) => {
  try {
    ...
    const response = await db
      .promise()
      .query(
        `UPDATE users SET password = '${req.body.password}' WHERE username = '${req.body.username}'`
      );
